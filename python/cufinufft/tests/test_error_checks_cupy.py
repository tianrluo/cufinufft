...
def test_set_nu_raises_on_dtype():
    ...
    kxyz_gpu = cp.array(kxyz)

    # Here we'll intentionally contruct an incorrect array dtype.
    kxyz_gpu_wrong_type = cp.array(kxyz.astype(cp.float64))
...
def test_exec_raises_on_dtype():
    ...
    c_gpu_wrong_dtype = cp.array(c.real)

    kxyz_gpu = cp.array(kxyz)
    fk_gpu = cp.empty(shape, dtype=complex_dtype)
    # Here we'll intentionally contruct an incorrect array dtype.
    fk_gpu_wrong_dtype = cp.empty(shape, dtype=np.complex128)
